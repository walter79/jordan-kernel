/*
 * drivers/cpufreq/cpufreq_boosted.c
 *
 * Copyright (C) 2012 AOSP, CyanogenDefy
 *
 ...
 * Modified to PowerBoost Standard by BMc08GT
 *
 * Sysfs fix and renamed to "boosted" by Epsylon3
 *
 * MOTO Devices: require symsearch.ko
 *
 *
 * For a general overview see the cpufreq documentation in
...
static DEFINE_PER_CPU(struct boosted_info_s, boosted_info);
...
#define dprintk(flag,msg...) do { \
	if (debug_mask & flag) pr_debug(msg); \
	} while (0)
...
static int cpufreq_governor_boosted(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_BOOSTED
static
#endif
struct cpufreq_governor cpufreq_gov_boosted = {
	.name = "boosted",
	.governor = cpufreq_governor_boosted,
	.max_transition_latency = 6000000,
	.owner = THIS_MODULE,
};

inline static void boosted_update_min_max(struct boosted_info_s *this_boosted, struct cpufreq_policy *policy, int suspend) {
	if (suspend) {
		this_boosted->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
			policy->max > sleep_ideal_freq ?
			(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
	} else {
		this_boosted->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
			policy->min < awake_ideal_freq ?
			(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
	}
}

inline static void boosted_update_min_max_allcpus(void) {
	unsigned int i;
	for_each_online_cpu(i) {
		struct boosted_info_s *this_boosted = &per_cpu(boosted_info, i);
		if (this_boosted->enable)
			boosted_update_min_max(this_boosted,this_boosted->cur_policy,suspended);
	}
}
...
inline static void reset_timer(unsigned long cpu, struct boosted_info_s *this_boosted) {
	this_boosted->time_in_idle = get_cpu_idle_time_us(cpu, &this_boosted->idle_exit_time);
	mod_timer(&this_boosted->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct boosted_info_s *this_boosted,
			      int new_freq, int old_freq, int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_boosted->freq_table;
	    ...
	    !cpufreq_frequency_table_target(policy,table,new_freq,prefered_relation,&index))
	{
		...
		if (target == old_freq) {
			...
			pr_warning(TAG ": frequency change failed: %d to %d => %d\n",
	...
	dprintk(DEBUG_JUMPS,TAG ": jumping from %d to %d => %d (%d)\n",
		old_freq,new_freq,target,policy->cur);

	return target;
}

static void cpufreq_boosted_timer(unsigned long cpu)
{
	u64 delta_idle;
	u64 delta_time;
	int cpu_load;
	...
	u64 now_idle;
	int queued_work = 0;
	struct boosted_info_s *this_boosted = &per_cpu(boosted_info, cpu);
	struct cpufreq_policy *policy = this_boosted->cur_policy;

	now_idle = get_cpu_idle_time_us(cpu, &update_time);
	old_freq = policy->cur;

	if (this_boosted->idle_exit_time == 0 || update_time == this_boosted->idle_exit_time)
		return;

	delta_idle = cputime64_sub(now_idle, this_boosted->time_in_idle);
	delta_time = cputime64_sub(update_time, this_boosted->idle_exit_time);

	// If timer ran less than 1ms after short-term sample started, retry.
	if (delta_time < 1000) {
		if (!timer_pending(&this_boosted->timer))
			reset_timer(cpu,this_boosted);
		return;
	}
	...
	dprintk(DEBUG_LOAD,TAG "T @ %d: load %d (delta_time %llu)\n",
		old_freq,cpu_load,delta_time);

	this_boosted->cur_cpu_load = cpu_load;
	this_boosted->old_freq = old_freq;
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_boosted->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_boosted->freq_change_time) >= up_rate_us))
		{
			dprintk(DEBUG_ALG,TAG "T @ %d ramp up: load %d (delta_idle %llu)\n",
				old_freq,cpu_load,delta_idle);
			this_boosted->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		}
		else this_boosted->ramp_dir = 0;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_boosted->ideal_speed ||
		  cputime64_sub(update_time, this_boosted->freq_change_time) >= down_rate_us))
	{
		dprintk(DEBUG_ALG,TAG "T @ %d ramp down: load %d (delta_idle %llu)\n",
			old_freq,cpu_load,delta_idle);
		this_boosted->ramp_dir = -1;
		work_cpumask_set(cpu);
...
{
	struct boosted_info_s *this_boosted = &per_cpu(boosted_info, smp_processor_id());
	struct cpufreq_policy *policy = this_boosted->cur_policy;
	...
	if (policy->cur == policy->min && timer_pending(&this_boosted->timer))
		del_timer(&this_boosted->timer);

	pm_idle_old();

	if (!timer_pending(&this_boosted->timer))
		reset_timer(smp_processor_id(), this_boosted);
}

/* We use the same work function to sale up and down */
static void cpufreq_boosted_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	int new_freq;
	int old_freq;
	int ramp_dir;
	struct boosted_info_s *this_boosted;
	struct cpufreq_policy *policy;
	unsigned int relation = CPUFREQ_RELATION_L;
	for_each_possible_cpu(cpu) {
		this_boosted = &per_cpu(boosted_info, cpu);
		if (!work_cpumask_test_and_clear(cpu))
			continue;

		ramp_dir = this_boosted->ramp_dir;
		this_boosted->ramp_dir = 0;

		old_freq = this_boosted->old_freq;
		policy = this_boosted->cur_policy;

		if (old_freq != policy->cur) {
			// frequency was changed by someone else?
			pr_warning(TAG ": frequency changed by 3rd party: %d to %d\n",
				old_freq,policy->cur);
			new_freq = old_freq;
		}
		else if (ramp_dir > 0 && nr_running() > 1) {
			// ramp up logic:
			if (old_freq < this_boosted->ideal_speed)
				new_freq = this_boosted->ideal_speed;
			...
			dprintk(DEBUG_ALG,TAG "Q @ %d ramp up: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_boosted->ideal_speed);
		}
		else if (ramp_dir < 0) {
			// ramp down logic:
			if (old_freq > this_boosted->ideal_speed) {
				...
				new_freq = old_freq * this_boosted->cur_cpu_load / max_cpu_load;
				if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
			}
			dprintk(DEBUG_ALG,TAG "Q @ %d ramp down: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_boosted->ideal_speed);
		}
		else {
			...
			new_freq = old_freq;
			dprintk(DEBUG_ALG,TAG "Q @ %d nothing: ramp_dir=%d nr_running=%lu\n",
				old_freq,ramp_dir,nr_running());
		}

		// do actual ramp up (returns 0, if frequency change failed):
		new_freq = target_freq(policy,this_boosted,new_freq,old_freq,relation);
		if (new_freq)
			this_boosted->freq_change_time_in_idle =
				get_cpu_idle_time_us(cpu,&this_boosted->freq_change_time);
		...
		else if (timer_pending(&this_boosted->timer))
			del_timer(&this_boosted->timer);
	}
}
...
{
	unsigned long input;
	ssize_t res = strict_strtoul(buf, 10, &input);
	if (res >= 0)
		debug_mask = input;
	return count;
}
...
static ssize_t store_sleep_ideal_freq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	...
	if (res >= 0 && input >= 0) {
		...
		if (suspended)
			boosted_update_min_max_allcpus();
	}
	return count;
}
...
static struct attribute * boosted_attributes[] = {
...
static struct attribute_group boosted_attr_group = {
	.attrs = boosted_attributes,
	.name = "boosted",
};

static int cpufreq_governor_boosted(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_boosted->cur_policy = new_policy;

		this_boosted->enable = 1;

		boosted_update_min_max(this_boosted,new_policy,suspended);

		this_boosted->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_boosted->freq_table)
			pr_warning(TAG ": no frequency table for cpu %d?!\n",cpu);
		...
		if (this_boosted->cur_policy->cur < new_policy->max && !timer_pending(&this_boosted->timer))
			reset_timer(cpu,this_boosted);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_boosted->cur_policy->cur > new_policy->max) {
			dprintk(DEBUG_JUMPS,TAG "I: jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_boosted->cur_policy,
						new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_boosted->cur_policy->cur < new_policy->min) {
			dprintk(DEBUG_JUMPS,TAG "I: jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_boosted->cur_policy,
	...
	case CPUFREQ_GOV_STOP:
		this_boosted->enable = 0;
		smp_wmb();
		del_timer(&this_boosted->timer);
		flush_work(&freq_scale_work);
		this_boosted->idle_exit_time = 0;
...
static void boosted_suspend(int cpu, int suspend)
{
	struct boosted_info_s *this_boosted = &per_cpu(boosted_info, smp_processor_id());
	struct cpufreq_policy *policy = this_boosted->cur_policy;
	...
	boosted_update_min_max(this_boosted,policy,suspend);
	if (!suspend) { // resume at max speed:
		new_freq = validate_freq(policy,sleep_wakeup_freq);

		dprintk(DEBUG_JUMPS,TAG "S: awaking at %d\n",new_freq);
		...
		dprintk(DEBUG_JUMPS,TAG "S: suspending at %d\n",policy->cur);
	}

	reset_timer(smp_processor_id(),this_boosted);
}

static void boosted_early_suspend(struct early_suspend *handler) {
	int i;
...
static void boosted_late_resume(struct early_suspend *handler) {
	int i;
...
static struct early_suspend boosted_power_suspend = {
	.suspend = boosted_early_suspend,
	.resume = boosted_late_resume,
#ifdef CONFIG_MACH_HERO
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
#endif
};

static int __init cpufreq_boosted_init(void)
{
	unsigned int i;
	struct boosted_info_s *this_boosted;
	...
	for_each_possible_cpu(i) {
		this_boosted = &per_cpu(boosted_info, i);
		this_boosted->enable = 0;
		this_boosted->cur_policy = 0;
		this_boosted->ramp_dir = 0;
		this_boosted->time_in_idle = 0;
		this_boosted->idle_exit_time = 0;
		this_boosted->freq_change_time = 0;
		this_boosted->freq_change_time_in_idle = 0;
		this_boosted->cur_cpu_load = 0;
		// intialize timer:
		init_timer_deferrable(&this_boosted->timer);
		this_boosted->timer.function = cpufreq_boosted_timer;
		this_boosted->timer.data = i;
	...
	INIT_WORK(&freq_scale_work, cpufreq_boosted_freq_change_time_work);

	register_early_suspend(&boosted_power_suspend);

	return cpufreq_register_governor(&cpufreq_gov_boosted);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_BOOSTED
fs_initcall(cpufreq_boosted_init);
#else
module_init(cpufreq_boosted_init);
#endif

static void __exit cpufreq_boosted_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_boosted);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_boosted_exit);

MODULE_AUTHOR ("Erasmux");
MODULE_AUTHOR ("modified by BMc08GT");
MODULE_AUTHOR ("fixed by Epsylon3");
MODULE_DESCRIPTION ("cpufreq_boosted - A smart cpufreq governor");
